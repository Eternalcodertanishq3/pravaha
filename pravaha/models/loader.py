...
class ModelLoader:
    ...
    def load(self, model_path: str, device: str = "cuda", dtype: str = "float16", quantization: Optional[str] = None, trust_remote_code: bool = False) -> tuple[Any, ArchConfig]:
        import torch
        ...
        kwargs: dict[str, Any] = {"torch_dtype": torch_dtype, "trust_remote_code": trust_remote_code, "device_map": "auto" if device == "cuda" else device}
        ...
        logger.info(f"Loading model: {model_path} (device={device}, dtype={dtype}, quant={quantization or 'none'})")
