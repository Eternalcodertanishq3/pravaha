...
class PrometheusMetrics:
    ...
    def __init__(self) -> None:
        try:
            ...
            self.requests_total = Counter(
                "pravaha_requests_total", "Total inference requests", ["model", "status"]
            )
            self.tokens_generated = Counter(
                "pravaha_tokens_generated_total", "Total tokens generated"
            )
            self.ttft_seconds = Histogram(
                "pravaha_ttft_seconds",
                "Time to first token",
                buckets=[0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0],
            )
            self.request_duration = Histogram(
                "pravaha_request_duration_seconds",
            ...
            self.audit_iterations = Counter(
                "pravaha_audit_iterations_total", "Total audit loop iterations"
            )
    ...
    def record_request(
        self, model: str, status: str, duration_s: float, ttft_s: float, tokens: int
    ) -> None:
    ...
    def update_gauges(
        self, tps: float, cache_pct: float, waiting: int, running: int, sessions: int
    ) -> None:
        if not self._available:
            return
