...
class PatchApplierAgent(BaseAgent):
    ...
    async def run(self, task: str, context: SharedContext, engine: Any) -> AgentOutput:
        ...
        if not all_issues:
            ...
            return AgentOutput(
                role=self.role, output=original, confidence=1.0, duration_ms=duration
            )

        issue_text = "\n".join(
            f"- [{i.get('severity', 'major')}] {i.get('description', str(i))}"
        ...
        return AgentOutput(
            ...
            confidence=0.8 if patches_applied > 0 else 0.5,
