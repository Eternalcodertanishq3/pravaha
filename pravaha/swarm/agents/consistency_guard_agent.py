...
class ConsistencyGuardAgent(BaseAgent):
    ...
    async def run(self, task: str, context: SharedContext, engine: Any) -> AgentOutput:
        ...
        if len(outputs) < 2:
            ...
            return AgentOutput(role=self.role, output="PASS: Insufficient outputs to compare",
                               duration_ms=duration, confidence=1.0)
        ...
        if not isinstance(contradictions, list):
            contradictions = []
        issues = [{"type": "contradiction", "severity": "major",
        ...
        return AgentOutput(
            role=self.role,
            output="PASS: Outputs consistent" if consistent else f"FAIL: {len(issues)} contradiction(s)",
            tokens_used=self._total_tokens, duration_ms=duration,
            confidence=1.0 if consistent else 0.4, issues=issues,
