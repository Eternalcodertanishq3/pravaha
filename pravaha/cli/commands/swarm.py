...
def run_swarm(
    ...
    import httpx
    try:
        resp = httpx.post("http://localhost:8000/v1/swarm/run",
                          json={"prompt": task, "pipeline": pipeline,
                                "max_audit_iterations": max_iterations},
                          timeout=120.0)
        result = resp.json()
...
def list_agents() -> None:
    ...
    for name, cls in ALL_AGENTS.items():
        agent = cls()
        console.print(f"  {'●' if agent.priority >= 1 else '○'} {name:25s} "
